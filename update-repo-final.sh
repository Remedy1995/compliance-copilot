#!/bin/bash
# ============================================================
# Compliance Copilot — FINAL IN-PLACE UPDATE (run from inside repo)
# Usage: chmod +x update-repo-final.sh && ./update-repo-final.sh
# ============================================================

set -e

echo "📁 Writing all premium source files..."
...
# ════════════════════════════════════════════════════════════
# CRITICAL FIX: @import MUST come before @tailwind directives
# ════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════
# TAILWIND CONFIG — correct content paths
# ════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════
# NEXT CONFIG — allow Google Fonts
# ════════════════════════════════════════════════════════════
...
async function callCompleteDevAgent({
  ...
  agentTag: string; systemPrompt: string; userPrompt: string;
  ...
  if (!response.ok) {
    console.error('[complete.dev API error]', await response.text());
...
git add .
git commit -m "fix: CSS @import order + complete premium UI v3

- FIXED: @import before @tailwind (was causing broken styles)
- FIXED: next.config.ts security headers
- FIXED: auth-middleware allows demo-token for testing
- FIXED: API route runtime changed to nodejs (SSE compatibility)
- Premium dark sidebar with gradient background
- Glassmorphism auth pages (dark theme + blur effects)
...
echo ""
echo "🔑 Ensure these env vars are set in Vercel Dashboard:"
